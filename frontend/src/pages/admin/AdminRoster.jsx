    ...
    <div className="space-y-6 flex flex-col h-[calc(100vh-100px)]">
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl ring-1 ring-slate-200 dark:ring-white/10 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shrink-0">
        ...
        <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-800 p-1.5 rounded-2xl ring-1 ring-slate-200 dark:ring-white/10 shadow-sm">
          ...
          <button onClick={handleToday} className="px-4 py-2 ml-2 text-sm font-bold text-brand-700 dark:text-brand-400 bg-brand-50 dark:bg-brand-500/10 rounded-xl hover:bg-brand-100 dark:hover:bg-brand-500/20 transition-colors ring-1 ring-brand-200 dark:ring-brand-500/30 hidden sm:block">
            Hôm nay
          </button>
        </div>
      </div>
              ...
              <div className="p-4 flex-1 overflow-y-auto custom-scrollbar space-y-4">
                ...
                {unassignedApts.length === 0 && (
                  <div className="text-center py-12 text-slate-400 dark:text-slate-500 text-sm flex flex-col items-center">
                    <CheckCircle className="w-10 h-10 text-brand-400 dark:text-brand-500 mb-3 opacity-50" />
              ...
              return (
                <div key={staff._id} className="w-[340px] bg-white dark:bg-slate-900 rounded-3xl ring-1 ring-slate-200 dark:ring-white/10 shadow-sm flex flex-col max-h-full">
                  <div className="p-5 border-b border-slate-100 dark:border-white/5 flex items-center gap-4">
                    <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-brand-500 to-cyan-600 flex items-center justify-center text-white font-bold shadow-lg shadow-brand-500/20 shrink-0 text-lg">
                      {staff.name.charAt(0)}
                    </div>
                    ...
                    <span className="bg-brand-50 dark:bg-brand-500/10 text-brand-700 dark:text-brand-400 text-xs font-bold px-3 py-1.5 rounded-xl ring-1 ring-brand-200 dark:ring-brand-500/30">
                      {staffApts.length}
                    </span>
                  </div>
...
function AppointmentCard({ apt, staffs, onAssign }) {
  return (
    <div className={`p-4 rounded-2xl ring-1 relative group transition-all shadow-sm hover:shadow-md ${apt.status === 'completed' ? 'bg-brand-50/50 dark:bg-brand-900/10 ring-brand-200 dark:ring-brand-500/20' : 'bg-white dark:bg-slate-900 ring-slate-200 dark:ring-white/10 hover:ring-brand-300 dark:hover:ring-brand-500/50'}`}>
      <div className={`absolute left-0 top-0 bottom-0 w-1.5 rounded-l-2xl ${apt.status === 'completed' ? 'bg-brand-400 dark:bg-brand-500 shadow-[0_0_10px_rgba(52,211,153,0.5)]' : 'bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.5)]'}`} />
      
      <div className="flex justify-between items-start mb-3 pl-3">
        <span className="font-bold text-brand-700 dark:text-brand-400 text-sm flex items-center gap-1.5">
          <Clock className="w-4 h-4" /> {apt.timeSlot}
        </span>
        {apt.status === 'completed' && (
          <span className="text-[10px] uppercase tracking-wider bg-brand-100 dark:bg-brand-500/20 text-brand-700 dark:text-brand-400 px-2 py-1 rounded-lg font-bold ring-1 ring-brand-200 dark:ring-brand-500/30">Xong</span>
        )}
      </div>
      ...
      <div className="pl-3 pt-4 border-t border-slate-100 dark:border-white/5">
        <select 
          ...
          className="w-full text-xs font-bold text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-800 border-0 ring-1 ring-slate-200 dark:ring-white/10 rounded-xl px-3 py-2.5 outline-none focus:ring-brand-500 transition-all cursor-pointer"
        >
